...
from ..strategies.cill_iwslt17 import (
...
def main(args):
    ...
    for i, exp in enumerate(experiences):
        eval_res = {}
        best_path = Path(args.model_save_path) / f"exp_{i}-best_model"
        model = T5ForConditionalGeneration.from_pretrained(best_path, return_dict=False)
        data_collator = DataCollatorForSeq2Seq(tokenizer=tokenizer, model=model)
        trainer = create_trainer(args, model, data_collator, tokenizer, training_args=training_args, exp=i)
        save_path = (
            Path(args.model_save_path) / f"exp_{i}_test_res-comet.json"
        )
        model.eval()
        for k, v in exp.test_datasets.items():
            # todo remove
            res = trainer.evaluate(eval_dataset=v)
            eval_res[k] = res
        with open(save_path, "w") as outfile:
            json.dump(eval_res, outfile, indent=4)
        logger.info(f"Scores on test sets for Experience {i}:\n {str(eval_res)}")
...
def create_trainer(args, model, data_collator, tokenizer, training_args, exp):
    # Metric
    metric = evaluate.load("sacrebleu")
    metric_comet = evaluate.load('comet')
    ignore_pad_token_for_loss = True
    pred_dir= Path(args.model_save_path) / "predictions"
    Path(pred_dir).mkdir(parents=True, exist_ok=True)
    path_inputs = pred_dir / f"exp_{exp}inputs.txt"
    path_preds = pred_dir / f"exp_{exp}preds.txt"
    path_targets = pred_dir / f"exp_{exp}targets.txt"
    ...
    def compute_metrics_comet(eval_preds):
        ...
        with open(path_inputs, "a") as f, open(path_preds, "a") as g, open(path_targets, "a") as h:
            for i in range(len(decoded_inputs)):
                f.write(decoded_inputs[i] + '\n')
                g.write(decoded_preds[i] + '\n')
                h.write(decoded_labels[i] + '\n')
        return result
