...
from ..cl_controllers.replay_controller import ReplayController
import wandb
import pandas as pd
...
def main(args):
    ...
    def log_translation(sources, targets, preds):
        ...
        new_transl = pd.DataFrame(
            {"source": _sources, "pred": _preds, "target": _targets}
        )
    ...
    cl_controller = ReplayController(
        model=model,
