...
from .logging_integration import get_reporting_integration_callbacks
# isort: on
...
from .cl_trainer_callback import (
    DefaultFlowCallback,
    PrinterCallback,
    ProgressCallback,
    TrainerCallback,
...
class CLTrainerV2:
    ...
    def __init__(
        ...
        self.callback_handler = CLCallbackHandler(
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = CLTrainerV2.get_optimizer_cls_and_kwargs(self.args)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    self.control = self.callback_handler.before_optim_step(
                    ...
                    self.control = self.callback_handler.after_optim_step(
                        args, self.state, self.control
                    )
                    model.zero_grad()
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        self.step_loss = loss
        self.control = self.callback_handler.before_backward(
        ...
        self.control = self.callback_handler.after_backward(
            self.args, self.state, self.control
        )
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        self.control = self.callback_handler.before_forward(
            self.args, self.state, self.control
        )
        outputs = model(**inputs)
        self.control = self.callback_handler.after_forward(
            self.args, self.state, self.control
        )
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            self.control = self.callback_handler.before_eval_fwd(
                args, self.state, self.control
            )
            ...
            self.control = self.callback_handler.after_eval_fwd(
                args, self.state, self.control
            )
